...
В том же `Query Tool` откройте и выполните `database/seed.sql`. Он создаст тестовый аккаунт, демо-приложение, несколько параметров и тестовый эксперимент.
...
Принадлежность приложения администратору хранится в поле `applications.account_id`. Поэтому у одного администратора может быть несколько приложений, но каждое приложение принадлежит только одному аккаунту.
